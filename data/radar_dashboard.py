...
# Streamlit 캐시 설정
@st.cache_data(ttl=3600, show_spinner=False)  # 1시간 캐시, 스피너 숨김
def load_data():
    """데이터 로드 및 전처리"""
    try:
        if not os.path.exists('radar_data.csv'):
            st.error("radar_data.csv 파일을 찾을 수 없습니다.")
            return pd.DataFrame()
...
def create_trend_chart(data, selected_dong, years):
    ...
    try:
        ...
        fig.update_yaxes(title_text="만원", row=1, col=2)
        fig.update_yaxes(title_text="%", row=2, col=1)
        fig.update_yaxes(title_text="년", row=2, col=2)
        
        return fig
    except Exception as e:
        st.error(f"트렌드 차트 생성 중 오류: {e}")
        return None
...
def main():
    ...
    if data.empty:
        st.error("데이터를 불러올 수 없습니다. radar_data.csv 파일이 현재 디렉토리에 있는지 확인해주세요.")
        return
    ...
    with st.sidebar:
        ...
        # 비교할 동네들 선택
        st.subheader("동네 비교")
        comparison_dongs = st.multiselect(
            "비교할 동네들 선택 (최대 6개)",
            available_dongs,
        ...
        if len(comparison_dongs) > 6:
            st.warning("최대 6개 동네까지만 선택 가능합니다.")
            ...
            if dong_data[required_cols].notna().all().all():
                ...
                if radar_fig is not None:
                    ...
                    with metrics_col1:
                        ...
                        st.metric("임대료", f"{actual_values[1]:,.0f}만원")
                    with metrics_col2:
